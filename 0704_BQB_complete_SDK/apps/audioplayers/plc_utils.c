...
#if defined(CHIP_BEST1400) || defined(CHIP_BEST1402) || defined(CHIP_BEST2300P)
...
#define ENABLE_MULTI_HEADER_CHECK
...
#ifdef ENABLE_MULTI_HEADER_CHECK
// find next 01 x8 ad
static uint32_t find_next_header(uint8_t *buf, uint32_t len)
{
    // here we assume the header is ok, so we check from bytes3
    for (uint8_t i = 3; i < len - 3; i++) {
        if (buf[i] == 0x01 && (buf[i+1] & 0xf) == 0x08 && buf[i+2] == 0xad) {
            return i;
        }
    }
...
#endif
#ifdef ENABLE_MULTI_HEADER_CHECK
        else if (find_next_header(sbc_buf, MSBC_PKTSIZE) != MSBC_PKTSIZE) {
            plc_type = PLC_TYPE_MULTI_HEADER;
            st->last_seq_num = 0xff;
        }
#endif
#ifdef ENABLE_PAD_CHECK
