...
extern void app_voice_gfps_find(void);
...
static void gfps_set_find_my_buds(uint8_t cmd)
{
    ...
    if(GFPS_FIND_MY_BUDS_CMD_START == cmd)    //ring
    {
        app_voice_gfps_find();
    }
    else if(GFPS_FIND_MY_BUDS_CMD_STOP == cmd)   //stop ring
    {
...
static void gfps_find_devices_ring_timeout_handler(void const *param)
{
    ...
    app_bt_start_custom_function_in_bt_thread(GFPS_FIND_MY_BUDS_CMD_STOP, 0, \
...
static void fp_rfcomm_ring_request_handling(uint8_t *request,uint16_t datalen)
{
    TRACE(1,"%s,[RFCOMM][FMD] request",__func__);
    DUMP8("%02x ", request, datalen);
    app_fp_msg_send_message_ack(FP_MSG_GROUP_DEVICE_ACTION, FP_MSG_DEVICE_ACTION_RING);
    if (datalen > 1)
    {
        fp_rfcomm_ring_timer_set(request[1]);
    }

    gfps_set_find_my_buds(request[0]);
...
#ifdef __BT_ONE_BRING_TWO__
        btif_spp_server_register_already_connected_callback(app_spp_multiai_is_already_connected);
#endif
