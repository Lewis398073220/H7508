...
typedef int32_t REF_PCM_T;
#else
typedef int16_t REF_PCM_T;
#endif

REF_PCM_T *aec_echo_buf = NULL;

// Use to free buffer
static REF_PCM_T *aec_echo_buf_ptr;
                     ...
                     uint8_t *buf, int len)
{
    ...
    aec_echo_buf = (REF_PCM_T *)speech_calloc(frame_len, sizeof(REF_PCM_T));
...
#if defined(SPEECH_TX_24BIT)
    int32_t *pcm32_buf = (int32_t *)pcm_buf;

    for (int i = 0; i < pcm16_len; i++) {
        pcm16_buf[i] = pcm32_buf[i] >> 8;
    }

    int32_t *ref32_buf = (int32_t *)ref_buf;
    for (int i = 0; i < pcm16_len / SPEECH_CODEC_CAPTURE_CHANNEL_NUM; i++) {
        ref16_buf[i] = ref32_buf[i] >> 8;
    }
#endif
...
#if defined(SPEECH_TX_24BIT)
    for (int i = pcm16_len - 1; i >= 0; i--) {
        pcm32_buf[i] = pcm16_buf[i] << 8;
    }
#endif
