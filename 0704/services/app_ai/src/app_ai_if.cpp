...
void app_ai_if_update_ble_conn_parameter(uint8_t  conidx,
...
{
#ifdef __AI_VOICE__
    app_ai_update_conn_parameter(conidx,
                                 min_interval_in_ms,
                                 max_interval_in_ms,
...
#endif
#ifdef BISTO_ENABLED
//        gsound_mobile_type_get_callback(type, _addr->address);
#endif
     }
...
{
#ifdef ANC_APP
    return isMusicOrPromptOnGoing||app_anc_assist_is_enable();
#else
...
uint32_t app_ai_if_get_music_or_prompt_sample_rate(void)
{
    ...
    else if (app_anc_assist_is_enable())
    {
...
bool app_spp_multiai_is_already_connected(int8_t server_channel)
{
    bool nRet = false;
    TRACE(2,"%s server_channel = %d",__func__,server_channel);
#ifdef BISTO_ENABLED
    if(nRet == false){
        nRet = app_spp_gsound_is_already_connected(server_channel);
    }
#endif
#ifdef __AI_VOICE__
    if(nRet == false){
        nRet = app_spp_ai_is_already_connected(server_channel);
    }
#endif
#ifdef GFPS_ENABLED
    if(nRet == false){
        nRet = app_spp_gfps_is_already_connected(server_channel);
    }
#endif
    return nRet;
}
