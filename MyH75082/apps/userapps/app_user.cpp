...
static uint8_t auto_powoff=0xFF;
...
void app_eq_custom_para_get(uint16_t customization_eq_value[10])
{
	...
	for(uint8_t i=0;i<10;i++){
		if(eq_custom_para_ancon.param[i].gain<0){
			temp=(int8_t)(2*(eq_custom_para_ancon.param[i].gain));
			temp*=-1;
			customization_eq_value[i]=(uint8_t)temp;
			customization_eq_value[i]|=0x80;
		}
		else{
			customization_eq_value[i]=(uint8_t)(2*(eq_custom_para_ancon.param[i].gain));
		}
...
void app_nvrecord_eq_param_set(uint16_t customization_eq_value[10])
{
	...
	for(i=0;i<10;i++){
		if(customization_eq_value[i]>0x8000){
			temp=(int16_t)(0x7fff & customization_eq_value[i]);
			temp*=-1;
			...
			temp=(int16_t)customization_eq_value[i];
			eq_custom_para_ancon.param[i].gain=(float)temp/100;
			//eq_custom_para.param[i].gain=(eq_custom_para.param[i].gain)/2;
			//if(eq_custom_para.param[i].gain>5)
				//eq_custom_para.param[i].gain=5;		
...
void app_nvrecord_para_get(void)
{
	...
	bt_name_len=nvrecord_env->bt_name_len;
